function buildRow(key, value) {
    ...
    var td = document.createElement("td"); //crate a cell for the key
    td.textContent = key+":";              //add the key text
    tr.appendChild(td);                    //then append it to the row
    td = document.createElement("td");     //crete a cell for the value
    td.textContent = value;                // add the vakue text
    tr.appendChild(td);                    //append value cell to the row
    return(tr);                            //return the row - inside showGeneData() it wil be appended to the table
   }
...
function showGeneData(id) {
    ...
    request.onreadystatechange = function(){
        if (this.readyState == 4 && (this.status >= 200 && this.status < 400)) {
            ...
            new_table.appendChild(buildRow("Entity", data.entity));  //we build a row for each key-value pair that came from the db
            ...
            //part of the code that allowws to update or delete gene
...
function writeGeneMenu() {
    ...
    request.onreadystatechange = function() {
        if (this.readyState == 4 && (this.status >= 200 && this.status < 400)){
            ...
            var th_row = document.createElement("tr");
            var th = document.createElement("th");
            th.textContent = "Gene ID"; //content at the top of our tabe
            th_row.appendChild(th);
            new_table.appendChild(th_row);            
...
function writeInitialContent() {
    ...
    contentHTML += "<br />\n";
    //content_region is called via DOM using getElementId ()c function
    content_region = document.getElementById("dynamic_content");
    //insert into content region contentHTML created above
